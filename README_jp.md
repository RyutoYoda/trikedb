...
# 自動保存は1変更ごとにファイル全体を書き直す。数件なら正しく、大量投入では二次。
# まとめて1回の保存にする。
with db.batch():
...
埋め込みは文単位でキャッシュディレクトリ（`TRIKEDB_CACHE_DIR`、なければ `~/.cache/trikedb`）にキャッシュされます — グラフの隣には置かないので、バイナリがdiffに混ざりません。27.5k文で初回 10.4秒、以降 0.11秒、書き込み直後でも 0.10秒。1件足したときに再エンコードするのはその1文だけで、コーパス全体ではないからです。本文を丸ごと持つプロパティはプレビュー＋マッチした箇所として返り、全文は `get_node` 側に残ります。
