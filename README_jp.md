...
- **抽出パイプラインではない。** 言語モデルを呼ばないし、鍵も預からないし、PDF も
  読まない。やるのは、あなたのオントロジーからプロンプトを書くことと、書き込みの前に
  全行をそれに照らして裁くこと。モデルも判断もあなたのものだ。抽出したグラフは
  ハルシネーションを受け継ぐ。こちらは、それが着地する前に見えるようにする側だ。
  trikedb 自身が動かすモデルは、任意の `[semantic]` 検索が使う小さな埋め込みモデル
  1つだけ — 初回に取得して、あとはローカルで動く。
