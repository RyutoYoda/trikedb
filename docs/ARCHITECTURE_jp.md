...
`model` は「事実とは何か」。`rules` / `rdf` / `reasoning` が文書の意味を、`storage` / `persistence` が入出力を、`audit` が出来上がった文書の読み取りを、`merge` が「これから入ってくる一括分がこの文書に何をするか」を担当します。`db` はそれらを束ねたストア — メソッドは各モジュールへの委譲で、モジュール側はストアを第一引数で受け取り、`db` を import し返しません。`html` / `importers` / `extract` / `embeddings` はコアの**上**に置いています: グラフが、それを描くページや、そこから組み立てられるファイル形式や、抽出器や、入っているとは限らない埋め込みモデルに依存してはいけないからです。`prompts` は `templates` と並んで最下層です — プロンプトはデータなので、抽出精度の変更が人の読める差分として届きます。`cli` / `mcp_server` / `serve` がその上の入口です。

import は上の層から厳密に下の層へ、一方向だけ。関数内 import は任意アダプタを任意のままにする手段なので依存には数えません（`db.to_html()` は呼ばれたときに `html` を、`db.extract()` は `extract` を import します）。`extract.py` については、標準ライブラリと trikedb 自身以外を import したら落ちる回帰テストも置いています: モデルプロバイダを同梱しないことこそが機能であり、何も検査していない保証は、誰かが import を足すまでの保証だからです。`tests/test_architecture.py` が層を宣言し、コードが合わなくなればビルドが落ちます — 層を決めずに新しいモジュールを足した場合も含めて。**宣言し、それを強制する**、このライブラリがオントロジーについて主張しているのと同じことを、リポジトリ自身にも適用しています。
...
重要な処理は、特別なAIではなく普通のPythonコードです。対応するAPIからの
書き込みは、次の順番で検査されます。

```python
self._check_predicate(p)          # 関係名が宣言されているか
self._check_link(s, p, o)         # 両端の種類と向きが合っているか
self._check_action(triple)        # 時刻・前提条件・担当者が正しいか
```

最初は、関係名が宣言一覧にあるかを調べます。次に、宣言された`domain`（関係の
始点に許される種類）と`range`（終点に許される種類）を、両端のノードの種類と
照合します。最後に`by`、`requires`、時刻を確認します。検査に失敗すると
`OntologyError`というエラーになり、新しい事実を保存する前に止まります。

前提条件の確認にLLMは使いません。TrikeDBは「関係名＋端点」を使った検索用の
索引を持ち、必要な過去の事実を探して時刻を比較します。前提が複数段なら、
`(主語・関係・目的語)`の固定長パターンをつないで照合します。batchの途中で、
後から届く入力によって結果が変わり得るものは一度保留し、最後にもう一度確認します。
それでも失敗したら、batch全体を元の状態に戻します。

`rdflib`と`Oxigraph`が裏でこの判定をしているわけではありません。これらは
RDF/SPARQLのクエリや投影を実行するエンジンで、書き込みガード本体は`rules.py`です。
