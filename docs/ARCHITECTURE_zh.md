...
关键路径不是特殊的AI，而是普通的Python代码。来自受支持API的写入会按以下顺序
检查：

```python
self._check_predicate(p)          # 关系名称是否已声明
self._check_link(s, p, o)         # 两端类型和方向是否匹配
self._check_action(triple)        # 时间、前置条件、执行者是否有效
```

第一步检查关系名称是否在声明列表中。第二步读取声明的`domain`（关系起点允许的
类型）和`range`（关系终点允许的类型），并与两个端点节点的类型比较。第三步检查
`by`、`requires`和时间。检查失败会抛出`OntologyError`，在新事实保存之前停止。

检查前置条件不调用LLM。TrikeDB维护按“关系名称＋端点”建立的搜索索引，寻找所需的
历史事实并比较时间。如果前置条件有多步，就连接固定长度的`(主语・关系・宾语)`模式
进行匹配。在batch中，可能因后续输入而改变结果的条件会暂存，并在最后重新检查；
仍然失败时，整个batch恢复到原来的状态。

`rdflib`和`Oxigraph`并不在后台替它做这个判断。它们负责执行RDF/SPARQL查询和相关
投影；写入守卫本身位于`rules.py`。
