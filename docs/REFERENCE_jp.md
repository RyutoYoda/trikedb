...
- アクションレイヤー: 時刻属性(`at:`・`when:`・`date:` など)付きのトリプルは
  ...
  履歴を新しい順で表示する。イベント自体は**起きた場所** — 2つのオブジェクトを
  結ぶ線の上 — にアクション色で描かれ、ラベルは日付と残した状態になる。
  線をクリックするとその主語ノードが開き、ヘッダの `events` ボタンで
  グラフ全体のログを時系列で読める。それ自体が実体でないイベント本文は
  赤ダイヤ(`--events AFFECTED_BY` で述語を固定)
