...
**文档本身不算事实之一。** 文档的头是文件里最醒目的字符串，所以模型第一个交还的
实体往往就是它 —— 而这一行恰恰是下游无论如何都修不回来的那一类：图里会长出一个
代表*文件*的节点，和这份文件所谈论的人与系统并排站着。因此提示词会点名这个头并
说明不要写它，并把同一条规则用在标题、章节号、图表题注、「本书」上，也用在事件上：
修订历史看起来和事件表一模一样，但它不是事件。头是从文档真实到达的形态里读出来的
—— Markdown 或 Word 任意层级的标题、Setext 下划线、YAML / TOML front matter、
转发邮件的 `Subject:`。没法诚实地点名时，它什么都不返回，而不是去猜。在 Python
一侧，这个检测就是 `trikedb.importers.document_title(text)`，而
`graph_filename(title)` 是这个头所对应的 YAML 名 —— 如果你要把这份文档的事实放进
一个独立的图，那里才是它该在的地方，而不是作为图中的一个节点。
`trikedb extract` 会把两者都打印出来。
