...
| **与文件的对比** | 要达到 `CLAUDE.md` 的准确率，所需 token 在 492 条时是 **1/8.7**，1,625 条时是 **1/78** |
...
| 492 条，文件是 82.0% / 9,668 token | 准确率 | token | |
|---|---|---|---|
| trikedb 15 条 | 77.0% | 409 | 低 5 个百分点 |
| **trikedb 50 条** | **86.0%** | **1,117** | **便宜 8.7 倍，高 4 个百分点** |
| trikedb 150 条 | 88.0% | 3,125 | 便宜 3.1 倍，高 6 个百分点 |

所以 492 条时同等条件下的数字是 **8.7 倍**，而不是第一张表原始 token 比所暗示的
24 倍。在 1,625 条时返回 15 条就已经与文件打平（72.0% 对 71.0%），那里的比值才是
78 倍。比值会拉开，是因为**变大的文件既更贵、准确率又更低**，图谱要跨过的门槛
反而降低了。
