...
| **与文件的对比** | 同样的事实放进 `CLAUDE.md` / `AGENTS.md`，在同样的准确率下每题要多花 **78 倍** 的 token |
...
![token 与准确率随语料规模的变化](memory_zh.png)

| 项目中的事实条数 | 整份知识文件 | 用 trikedb | token |
|---|---|---|---|
| 492 | 9,668 tok · 82.0% | 409 tok · 77.0% | 24 倍 |
| 1,181 | 22,153 tok · 72.0% | 397 tok · **73.0%** | 56 倍 |
| 1,625 | 30,354 tok · 71.0% | 389 tok · **72.0%** | **78 倍** |
...
那一节，得先把文件读完 —— 所以它的成本正比于*项目的大小*。图谱是可寻址的，成本
正比于*答案的大小*：无论语料是 492 条还是 3,998 条，送出去的都是约 15 条三元组。
...
它成本上升的一部分原因 —— 其余每一行都是一轮。每个数字的大部分是 harness 自身的
系统提示（Claude Code 在完全没有项目知识时也要 31,014 个 token），两组都躲不开。
把它减掉，**知识本身的成本是文件 +10,304 个 token，图谱 +363 个** —— 28 倍。
