...
**trikedb 做到了什么：** 在 **89.3%** 的题目上，它找到了答案并把它摆到模型面前，
每题耗时 **0.59 秒** — 没有服务器，没有索引，只有一个文件。这才是这个库要负责的
数字。

**这在下游值多少：** 把一个笔记本级别的 8B 阅读模型接到同样的检索上，答对
**77.7%**（没有图谱时是 **42.7%**）。89.3% 与 77.7% 之间的差距，全部是阅读模型把
递到手上的答案丢掉了，所以更强的阅读模型分数会更高 — 这个基准上已发表的领先者用的
是 GPT-4 级或微调过的阅读模型，落在 80 多的中段到高段。

**它在哪里停下：** 到 **10 万条三元组**仍然很快；最先撑不住的是语义检索，3 万条。
