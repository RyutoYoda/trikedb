...
ここでは、trikedbを実際にベンチマークした結果を示します。検索方法、回答率、入力トークン、処理時間を同じ条件で測定し、結果を比較しました。採点は、回答文に正解文字列が含まれているかを判定する方式です。公式WebQSPの順位を再現する採点方式ではありません。

同じ100問で、MD全文は中央値9,668トークン、trikedbから50件を渡した場合は1,117トークンでした。入力トークンは8,551トークン（88.4%）削減されています。回答率はMD全文82%、trikedb 86%でした。この4ポイント差には有意差がありません。条件と限界は[監査と再現性の記録](https://github.com/RyutoYoda/trikedb/blob/main/benchmarks/VALIDATION.md)にまとめています。

# ベンチマーク

測定では、8Bを300問、27Bを150問で比較しました。速度・精度・入力トークンを別々に測定し、グラフ条件にはgrounding指示を含めています。
...
| **ファイルとの比較** | 492件で入力中央値88.4%減、回答文字列の一致率86%対82%（有意差なし） |
...
この表は、全文を渡す方法と、trikedbから指定件数だけ取得する方法を比較します。
trikedbが返す件数は `--cap` で変更できます。件数を減らすと入力は短くなりますが、
必要な事実が入らず回答率が下がる場合があります。まず同じ `--cap` で再実行し、
回答率と入力トークンを一緒に確認してください:

| 492 facts、ファイルは 82.0% / 9,668 トークン | 精度 | トークン | 削減 |
|---|---|---|---|
| trikedb 15件 | 77.0% | 409 | 95.8%、ただし精度は5pt負け |
| **trikedb 50件** | **86.0%** | **1,117** | **88.4%、回答率の差は有意差なし** |
| trikedb 150件 | 88.0% | 3,125 | 67.7%、精度は6pt勝ち |

50件条件では、全文条件より入力トークンが88.4%少なく、回答率は86%対82%でした。
回答率の4ポイント差は有意差なしです。
...
- Hits@1は回答文全体に正解文字列が含まれているかを判定します。F1は改行区切り予測の一致率とgoldの再現率を使い、0〜1に収まります。WebQSP/RoG公式採点とは判定方法が異なるため、論文のスコアとは直接順位比較できません。
