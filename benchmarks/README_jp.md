...
| **ファイルとの比較** | `CLAUDE.md` と同じ精度に到達するのに必要なトークンは、492 facts で **8.7分の1**、1,625 facts で **78分の1** |
...
![コーパスサイズに対するトークンと精度](memory_jp.png)
...
| 492 facts、ファイルは 82.0% / 9,668 トークン | 精度 | トークン | |
|---|---|---|---|
| trikedb 15件 | 77.0% | 409 | 5pt 負け |
| **trikedb 50件** | **86.0%** | **1,117** | **8.7分の1、4pt 勝ち** |
| trikedb 150件 | 88.0% | 3,125 | 3.1分の1、6pt 勝ち |

つまり 492 facts での等価な数字は **8.7倍**で、最初の表の生のトークン比が示す24倍
ではありません。1,625 facts では15件で既にファイルに並んでいる（72.0% 対 71.0%）
ので、そこでの比が78倍。比が開くのは、**育ったファイルは高くなり、同時に精度も
落ちる**ので、グラフが超えるべき基準が下がるからです。
...
+10,304 トークン、グラフで +363 トークン** — 28倍の差です。
