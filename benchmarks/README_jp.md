...
ない — Solomon の節だけを渡すには、まず全文を読まなければならない。だから送る量は
*プロジェクトの大きさ*に比例します。グラフはアドレス可能なので、送る量は*答えの
...
ためで、トークンが増える理由の一部です。他はすべて1ターン。各数字の大半はハーネス
自身のシステムプロンプト（Claude Code は知識ゼロでも 31,014 トークン）で、これは
どちらの条件でも避けられません。それを引くと、**知識そのものが占めるのはファイルで
