...
**trikedbがやったこと:** 300問のうち **89.3%** で正解を見つけ、モデルの目の前に
置きました。1問あたり **0.59秒**。サーバもインデックスもなく、ファイル1つです。
このライブラリが責任を持つ数字はこれです。

**それが下流でいくらになるか:** ノートPCで動く8Bのリーダーを同じ検索に載せると、
**77.7%** が正解になります（グラフなしでは **42.7%**）。89.3%と77.7%の差は、渡した
のにリーダーが落とした分がすべてなので、強いリーダーならもっと上がります — この
ベンチマークで公開されている上位勢はGPT-4級かファインチューニング済みのリーダーを
使って80台半ば〜後半です。

**どこで止まるか:** **10万トリプル**まで速く、最初に苦しくなるのは意味検索で
3万からです。
