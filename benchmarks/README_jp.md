...
このページでは、trikedbの検索方法を同じ条件で比較できます。下のコマンドでベンチマークを実行し、同じ質問・同じモデル・同じ採点方法で結果を確認してください。採点は、回答に正解文字列が含まれるかを数える簡易指標です。公式WebQSPの順位を再現するものではありません。

参考として、同じ100問では「全文を渡す」条件が82%、「trikedbから50件を渡す」条件が86%でした。入力トークンの中央値は88.4%少なくなりました。再実行時は、この数値を基準値として、検索件数・質問数・モデル名をそろえて比較してください。条件の詳細は[監査と再現性の記録](https://github.com/RyutoYoda/trikedb/blob/main/benchmarks/VALIDATION.md)にあります。

# ベンチマーク

実行例では、8Bを300問、27Bを150問で比較します。速度・精度・入力トークンを別々に測定し、グラフ条件にはgrounding指示を含めます。
...
50件条件は、全文条件に対して入力トークンが88.4%少なく、回答率は86%対82%でした。
この差が一般のデータでも再現するとは限らないため、ベンチマークを実行するときは
検索件数、質問数、モデル名を記録してください。
