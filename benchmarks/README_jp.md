...
**trikedb は WebQSP 300問のうち 89.3% で、正解をモデルの目の前に置いた。1問あたり
0.59秒。** 小さなローカルのリーダーを付けると、端から端まででは 77.7% が正解に
なる — 同じモデルがグラフなしで出す 42.7% に対して。

![モデルと条件ごとの Hits@1](accuracy.png)

一方、リーダーのパラメータを3倍にしても、グラフがなければ何も変わらなかった
（44.0% 対 42.7%、対応のある検定で p = 1.0）。ここで精度を動かしているものは、
モデルサイズではない。

以下はすべて、その数字がどう作られたか、それが何を言っていないか、そして天井が
どこにあるかである。
...
| 同じものを `qwen3.8:27b` で | **70.36** | 約4,377トークン | 67.3% |
...
自然な次の一手はより大きなモデルなので、測った: `qwen3.8:27b`、パラメータ3.4倍、
同じ検索、同じプロンプト、同じ質問の最初の150問。

| 条件 | Hits@1 | F1 | n |
|---|---|---|---|
| `qwen3:8b` — グラフなし | 42.7% | 27.9% | 300 |
| `qwen3.8:27b` — グラフなし | 44.0% | 30.6% | 150 |
| `qwen3:8b` — グラフあり | **77.7%** | 57.4% | 300 |
| `qwen3.8:27b` — グラフあり | 67.3% | 57.1% | 150 |

結果は2つあり、2つ目は歩いて確かめる価値のある罠である。

**グラフがなければ、パラメータ3.4倍は何も変えない。** 同じ150問で対応を取ると
2つのモデルはぴったり互角である — 14問ずつ、McNemar 検定で p = 1.0。どちらも
Freebase を暗記しておらず、大きいことはそれには効かない。これに対してグラフは
8B で +35ポイント（p = 9e-20）、27B で +23ポイント（p = 8e-05）の価値がある。
**ここで精度を動かしているものは、モデルサイズではない。**

**グラフがあると 8B の方が高い点を取る — そしてそれは能力の結果ではない。**
対応のある検定はその差が実在すると言う（27B が外した22問で8B が正解、8B が外した
4問で27B が正解、p = 0.0005）が、原因は能力ではなく「答えないこと」である —
27B は答えを出さずに「I don't know」と返す回数がはるかに多い:

| | 答えずに「I don't know」と返した | そのうち正解が文脈にあった | そのうち 8B は正解していた |
|---|---|---|---|
| `qwen3:8b` | 4 / 150 | — | — |
| `qwen3.8:27b` | **30 / 150** | 19 | 13 |

プロンプトには*「分からなければ、正確に I don't know と答えよ」*と書いてあり、
27B はそれをはるかに厳格に守る — 質問の20%で答えを出さず、そのほとんどは正解が
目の前に座っていた。Hits@1 は「正解を含んでいるか」を見るので、答えなければ必ず
0点である。両モデルが実際に答えた120問に限ると、差は検出できなくなる:
88.3% 対 84.2%、p = 0.27。F1 はどちらもほとんど動かない（57.4 対 57.1）が、
それも整合する: 答えないことは Hits@1 を丸ごと削り、F1 は与えられた回答だけを
採点するからである。

**だからこのベンチマークは2つのモデルを順位付けできない。** 測ったのは1つの
プロンプトと2つのモデルの相互作用であり、誠実な読み方はこうである:

- グラフは両方で有意に効き、その効き方はサイズよりはるかに大きい
- より大きなリーダーは無料のアップグレードではない — 同じ指示を違うように
  受け取りうるし、ここではそれが Hits@1 を12ポイント削った
- リーダーを差し替えるなら、「分からなければそう言え」の指示を再調整して測り直す
  こと。`--model` が
  それをフラグ1つの変更にしている。2つの半分を分離可能に保っている理由がこれで
  ある

しかも低い点数のために3.4倍のレイテンシを払っている。同じ発見の実用版である。
