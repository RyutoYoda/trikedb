...
嵌入按句子缓存在你的缓存目录里（`TRIKEDB_CACHE_DIR`，否则 `~/.cache/trikedb`）— 绝不放在图谱旁边，所以不会有二进制文件混进你的 diff。27.5k 句：首次 10.4 秒，之后 0.11 秒，写入之后也只要 0.10 秒，因为新增一条事实只重新编码那一条，而不是整个语料。属性里装着整篇文档时，返回的是预览加上真正匹配到的那一段；全文仍然在 `get_node` 那边。
